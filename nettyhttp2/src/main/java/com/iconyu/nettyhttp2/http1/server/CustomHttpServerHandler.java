package com.iconyu.nettyhttp2.http1.server;
...
public class CustomHttpServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Object msg) {
        ...
        responseData.append(com.iconyu.nettyhttp2.http1.server.RequestUtils.evaluateDecoderResult(request));

        if (msg instanceof HttpContent) {
            HttpContent httpContent = (HttpContent) msg;

            responseData.append(com.iconyu.nettyhttp2.http1.server.RequestUtils.formatBody(httpContent));
            responseData.append(com.iconyu.nettyhttp2.http1.server.RequestUtils.evaluateDecoderResult(request));

            if (msg instanceof LastHttpContent) {
                LastHttpContent trailer = (LastHttpContent) msg;
                responseData.append(com.iconyu.nettyhttp2.http1.server.RequestUtils.prepareLastResponse(request, trailer));
